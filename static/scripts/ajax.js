$(document).ready(function(){
    ...
    $(".input-area").submit(function(e){
        ...
        if (text === m ){
            choice = "yes"
            model_no = ""
            msg_append("Choices resseted",botimg,"left","Healthcare Chatbot");
            console.log(choice);
            ...
            msg_append(after_opt[parseInt(model_no)-1],botimg,"left","Healthcare chatbot")
            choice = "no"
            console.log(choice)
        }
            ...
            $.ajax({
                ...
                if (data.modelno === "1"){
                    if(data.response.length === 1){
                        msg_append("One medicine found in our database", botimg, "left", "Healthcare chatbot")
                        medicine_append(data.response , botimg, "left", "Healthcare chatbot",1)
                    }
                    else if (data.response.length > 1){
                        msg_append("More than one medicine found in our database ", botimg, "left", "Healthcare chatbot")
                        medicine_append(data.response,botimg,"left","Healthcare chatbot",1)
                    }
                    else {
                        msg_append("No medicine found, try being more specific ", botimg, "left", "Healthcare chatbot")
                    }
                }
                    ...
                    if (data.response.length === 1){
                        msg_append("One medicine found in our database", botimg, "left", "Healthcare chatbot")
                        medicine_append(data.response , botimg, "left", "Healthcare chatbot",2)
                    }
                    else if (data.response.length > 1){
                        msg_append("More than one medicine found in our database ", botimg, "left", "Healthcare chatbot")
                        medicine_append(data.response,botimg,"left","Healthcare chatbot",2)
                    ...
                    if (data.response.length === 1){
                    msg_append("The predicted disease is: "+ data.response[0]["Disease"],botimg,"left","Healthcare chatbot")
                    }
                    else if (data.response.length > 1){
                    dis = []
                    ...
                    msg_append("The predicted diseases are: "+ dis.toString() ,botimg,"left","Healthcare chatbot")
                    }
                    else{
                        msg_append("No disease found, try entering some more symptoms",botimg,"left","Healthcare chatbot")
                    }
                    }
                else if(data.modelno === "4"){
                    if (data.response.length === 1){
                        msg_append("Following are the symptoms for "+ data.response[0]["Disease"],botimg,"left","Healthcare chatbot")
                        symptom_append(data.response,botimg,"left","Healthcare chatbot")
                    }
                    else if (data.response.length > 1){
                        msg_append("More than one disease was predicted for the following query found" ,botimg,"left","Healthcare chatbot")
                        symptom_append(data.response,botimg,"left","Healthcare chatbot")

                    }
                    else{
                        msg_append("No disease found in the database, try beign more specific" ,botimg,"left","Healthcare chatbot")
                    }
                }
            ...
            msg_append(text,userimg,"right","Me")
            msg_append("You need to select what you want!!",botimg,"left","Healthcare chatbot");
        }
